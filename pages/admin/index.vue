			...
			<v-card class="mt-4">
				...
				<v-card-text class="text-md-center">
					...
					<v-chip color="red lighten-2" text-color="white">
						<v-avatar class="red">{{ getFanStatistics.value }}</v-avatar>
						{{ getFanStatistics.name }}
					</v-chip>
					<v-chip color="pink lighten-2" text-color="white" v-if="loadedStatistics.game && loadedStatistics.game.sports">
						<v-avatar class="pink">{{ loadedStatistics.game.sports.value }}</v-avatar>
						{{ loadedStatistics.game.sports.name }}
					</v-chip>
					<v-chip color="purple lighten-2" text-color="white" v-if="loadedStatistics.game && loadedStatistics.game.competitions">
						<v-avatar class="purple">{{ loadedStatistics.game.competitions.value }}</v-avatar>
						{{ loadedStatistics.game.competitions.name }}
					</v-chip>
					<v-chip color="deep-purple lighten-2" text-color="white" v-if="loadedStatistics.game && loadedStatistics.game.teams">
						<v-avatar class="deep-purple">{{ loadedStatistics.game.teams.value }}</v-avatar>
						{{ loadedStatistics.game.teams.name }}
					</v-chip>
					<v-chip color="indigo lighten-2" text-color="white" v-if="loadedStatistics.game && loadedStatistics.game.players">
						<v-avatar class="indigo">{{ loadedStatistics.game.players.value }}</v-avatar>
						{{ loadedStatistics.game.players.name }}
					</v-chip>
					<v-chip color="blue lighten-2" text-color="white" v-if="loadedStatistics.game && loadedStatistics.game.points">
						<v-avatar class="blue">{{ loadedStatistics.game.points.value }}</v-avatar>
						{{ loadedStatistics.game.points.name }}
					</v-chip>
				</v-card-text>
			</v-card>
				...
				<v-card-text>
					...
					>
						<template slot="items" slot-scope="props">
							...
							<td>
								<span v-for="(user, index) in props.item.users" :key="index">{{user}}<span v-if="index < props.item.users.length - 1">,&nbsp;</span></span>
							</td>
...
<script>
	...
	export default {
		...
		computed: {
			...
			getFanStatistics () {
				try {
					return this.loadedStatistics.game.fans
				} catch {
					return 'Fan statistics not found.'
				}
			}
		},
		methods: {
